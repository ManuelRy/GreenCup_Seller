        ...
        <div class="two-column-layout">
            ...
            <div class="gallery-card fade-in">
                ...
                <div class="photo-posts-container">
                    ...
                    @foreach($groupedPhotos as $groupIndex => $photoGroup)
                        <div class="photo-post-thumbnail" onclick="showPostDetails({{ $groupIndex }})">
                            <div class="post-thumbnail-container">
                                <div class="post-thumbnail-image">
                                    @php $firstPhoto = $photoGroup->first(); $photoUrl = getPhotoUrl($firstPhoto); @endphp
                                    <img src="{{ $photoUrl }}" 
                                         alt="{{ $firstPhoto->caption ?: 'Store photo' }}"
                                         onerror="this.style.display='none'; this.parentElement.classList.add('img-error');">
                                    @if(count($photoGroup) > 1)
                                        <div class="thumbnail-count">+{{ count($photoGroup) - 1 }}</div>
                                    @endif
                                    @if($firstPhoto->is_featured)
                                ...
                                <div class="post-thumbnail-info">
                                    <div class="thumbnail-title">
                                        @if($firstPhoto->caption)
                                            {{ limitString($firstPhoto->caption, 40) }}
                                        @else
                                            Store Photos
                                        @endif
                                    </div>
                                    <div class="thumbnail-meta">
                                        📅 {{ $firstPhoto->created_at->format('M j, Y') }} • 
                            ...
                            <div class="thumbnail-actions">
                                <button class="thumbnail-action-btn" onclick="showPostDetails({{ $groupIndex }}, event)" title="View Details">
                                    👁️
                                </button>
                                <button class="thumbnail-action-btn" onclick="editPhoto({{ $firstPhoto->id }}, event)" title="Edit">
                                    ✏️
                                </button>
                                <form action="{{ route('seller.photos.destroy', $firstPhoto->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this photo?')">
                                    @csrf
                                    @method('DELETE')
    ...
    @foreach($groupedPhotos as $groupIndex => $photoGroup)
        {
            ...
            created_at: "{{ $photoGroup->first()->created_at->format('M j, Y g:i A') }}",
            caption: "{{ $photoGroup->first()->caption ?: '' }}",
            featured_count: {{ $photoGroup->where('is_featured', true)->count() }},
            has_location: {{ ($photoGroup->first()->latitude && $photoGroup->first()->longitude) ? 'true' : 'false' }},
            location_address: "{{ $photoGroup->first()->location_address ?? '' }}",
            latitude: "{{ $photoGroup->first()->latitude ?? '' }}",
            longitude: "{{ $photoGroup->first()->longitude ?? '' }}"
        },
    @endforeach
];
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            ...
            function(error) {
                ...
                currentLocation = { lat: 11.5564, lng: 104.9282 };
...
function generateMapThumbnail(lat, lng) {
    ...
    const mapUrl = `https://api.mapbox.com/styles/v1/mapbox/streets-v11/static/pin-s-l+000(${lng},${lat})/${lng},${lat},14/160x160?access_token=pk.eyJ1IjoibWFwYm94IiwiYSI6ImNpejY4NXVycTA2emYycXBndHRqcmZ3N3gifQ.rJcFIG214AriISLbB6B5aw`;
...
function showPostDetails(groupIndex, event) {
    if (event) event.stopPropagation();
    
    const group = photoGroupsData[groupIndex];
    if (!group) return;
    ...
    // Set title
    title.textContent = group.caption || 'Store Photos';
    
    // Set meta information
    let metaHTML = `
        <div class="post-detail-meta-item">
            <span class="post-detail-meta-label">📅 Upload Date</span>
            <span class="post-detail-meta-value">${group.created_at}</span>
        </div>
        <div class="post-detail-meta-item">
            <span class="post-detail-meta-label">📷 Photos Count</span>
            <span class="post-detail-meta-value">${group.photos.length} photo${group.photos.length > 1 ? 's' : ''}</span>
        </div>
        <div class="post-detail-meta-item">
            <span class="post-detail-meta-label">⭐ Featured Photos</span>
            <span class="post-detail-meta-value">${group.featured_count} featured</span>
        </div>
        <div class="post-detail-meta-item">
            <span class="post-detail-meta-label">📂 Category</span>
            <span class="post-detail-meta-value">${group.photos[0].category}</span>
        </div>
    `;
    
    // Add location information if available
    if (group.has_location && group.latitude && group.longitude) {
        metaHTML += `
            <div class="post-detail-meta-item">
                <span class="post-detail-meta-label">📍 Location</span>
                <span class="post-detail-meta-value">
                    <a href="https://www.google.com/maps?q=${group.latitude},${group.longitude}&z=15&t=m" 
                       target="_blank" 
                       style="color: #00b09b; text-decoration: none;">
                        ${group.location_address || `${parseFloat(group.latitude).toFixed(4)}, ${parseFloat(group.longitude).toFixed(4)}`}
                        🔗
                    </a>
                </span>
            </div>
    ...
    group.photos.slice(0, photoCount === 3 ? 3 : photoCount > 4 ? 4 : photoCount).forEach((photo, index) => {
    ...
    // Set caption
    if (group.caption) {
        captionContent.textContent = group.caption;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    currentLocation = { lat: 11.5564, lng: 104.9282 }; // Phnom Penh coordinates
